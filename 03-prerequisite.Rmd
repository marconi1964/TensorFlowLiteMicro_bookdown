...
坊間許多的 Arduino 書/網站都有詳細的 Arduino IDE 安裝介紹, 例如 [台灣智能感測科技 Arduino IDE 安裝](https://www.taiwansensor.com.tw/lessons/arduino-arduino-ide-下載與安裝軟硬體/), 就不在此佔用篇幅, Arduino IDE 安裝完成後, 需要在 Arduino IDE 上安裝 Nano 33 BLE 硬體跟 TensorFlow Lite 的範例程式, 再把 g_model[ ] 複製貼上, 其餘步驟就跟一般的 Arduino 應用程式一樣. 簡單說明如下:  
...
3. 到 Arduino menu 的 File - Examples - Arduino_TensorFlowLite 找到並打開 hello_world (或其它應用) 程式. 打開後, 會同時展開好幾個程式, 找到 model.cpp 用 colab 訓練完成的 g_model[ ] 內容來取代現有 model.cpp 內的 g_model[ ], 即可按照一般 Arduino 的程式般下載到 Nano 33 BLE 後執行.
